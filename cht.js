...
preloadImages('images/cht-btn-left.png', 'images/cht-btn-center.png', 'images/cht-btn-right.png', 'images/cht-btn-left-act.png', 'images/cht-btn-center-act.png', 'images/cht-btn-right-act.png');
...
function chatConnectionChecker() {
    if (!lastMsgTime || !LMTS || reconnecting)
        return false;
...
function chatTotalReconnect() {
    if (reconnecting || !LMTS)
        return false;
    if (!LMTS.ever_authorized() && time_current() - lastMsgTime <= lastMsgMaxTimeDiff)
        return false;
...
function chatRefreshUsers(force) {
    ...
    if ($chat_user_list.find('li.area.selected').length && !force) {
        ...
        if ($('#chat_users_list .list_denied', frame).length)
            return false;
    }
...
function chatUpdateDataAttach() {
    ...
    if (!_top().frames['chat'] || !_top().frames['chat'].frames['chat_text'])
        return false;
    if (_top().frames['chat'].frames['chat_text'].document)
        obj = _top().frames['chat'].frames['chat_text'].document.getElementById('content');
    if (!obj) { // No frame
...
function chatReceiveMessage(msg) {
    ...
    if (!msg || msg.to_admin && !CHAT.admin)
        return;
    if (msg.delay) {
        var delay = 0;
    ...
    if (!chat_check_chat_text_ready(function () { chatReceiveMessage(msg); }))
        return false;
        ...
        if ((jQuery.inArray(msg.type, [msg_type.report, msg_type.report_answer]) != -1) && !jQuery.isEmptyObject(msg.report_msg)) {
            ...
            if (typeof (msg.report_msg.to_user_nicks) === 'object') {
                for (var to_user_id in msg.report_msg.to_user_nicks) {
                    if (parseInt(to_user_id))
                        nicks.push(msg.report_msg.to_user_nicks[to_user_id]);
                }
            }
            if (nicks.length)
                client_text += nicks.join(', ') + ': ';
    ...
    for (var i in chatOpts) {
        ...
        if ((msg.channel == channels.user) || opt.channel & msg.channel || msg.type == msg_type.system || msg.type == msg_type.broadcast) {
            if (msg.channel != channels.user && msg.channel != channels.aux && !(opt.channel & channels.area) && !msg.command && (msg.type == msg_type.system && !(opt.system_msgs && (msg.channel & opt.channel))) && !msg.urgent)
                continue;
            if (msg.type == msg_type.system && msg.command && !(opt.channel & msg.channel))
                continue;
...
var lastFightMessages = [];
var lastFightMessageIds = [];
var fightStarted = false;
var fightStartedTimeout = null;
...
function chat_load_emos_callback(data, page) {
    ...
    $pages.html(data.pages_html);
    if (typeof page != 'undefined')
        CHAT.smiles.emos_by_page[page] = data;
    $('#smile_div_bg', _top().document).add(smile_div).show();
}
;
...
function chat_get_class(channel) {
    var t_cl = '';
    if (channel == channels.area || channel == channels.area_subchannel)
        t_cl = 'cml_loc';
    else if (channel == channels.user)
        t_cl = 'cml_prv';
    else if (channel == channels.clan)
        t_cl = 'cml_cln';
    else if (channel == channels.trade)
        t_cl = 'cml_trd';
    else if (channel == channels.party)
        t_cl = 'cml_pty';
    else if (channel == channels.fight)
        t_cl = 'cml_fgt';
    else if (channel == channels.raid)
        t_cl = 'cml_rd';
    else if (channel == channels.ally)
        t_cl = 'cml_all';
    else if (channel == channels.aux)
        t_cl = 'cml_aux';
    return t_cl;
}
;
function chatFormatMessage(msg) {
    ...
    var enemy = false;
    if ((msg.user_kind && msg.user_kind != session.kind && !('user_kind_f' in msg)) || (msg.raid_team_num && session.raid && msg.raid_team_num != session.raid.substr(session.raid.length - 1)))
        enemy = true;
    var mymsg = false;
    ...
    if (!enemy && ((msg.channel == channels.user) || (msg.user_id == session.id) || (msg.to_user_ids && inarray(msg.to_user_ids, session.id))))
        mymsg = true;
    ...
    if (enemy)
        t_cl = 'cml_enm';
    else if (talkfree_enemy)
        t_cl = 'cml_tkf';
    else
        t_cl = chat_get_class(msg.channel);
    ...
    var t_add = '';
    if (msg.type == msg_type.def || msg.type == msg_type.report)
        t_add += ' oncontextmenu="return timeMnu(this,event,' + msg.id + ',1)" onmousedown="return timeMnu(this,event,' + msg.id + ',2)"';
...
function sessionUpdate(data) {
    ...
    if (!CHAT.ready)
        setTimeout(function () { sessionUpdate(data); }, 10);
    if ($channel_tabs === null)
...
function chat_timezone_init() {
    ...
    if (diff)
        chat_clock_time_shift(diff);
    else
        chat_clock_time_shift(-time_shifted);
    return true;
}
var time_shifted = 0;
...
function chat_update_text_size(text_size, just_user_list) {
    ...
    if (!just_user_list)
        $('html > head', _top().chat.chat_text.document).append(style.clone());
    $('html > head', _top().chat.chat_user.document).append(style);
}
;
function chat_change_timezone() {
    if (CHAT.timezone_diff == 0)
        return false;
...
function cht_settings_carousel_width() {
    var parent_w = $('.channel-tabs-container').parent().outerWidth(true), child_w = 0;
...
function chat_user_sort_compare_items(a, b, sortField, returnValue) {
    var tmp1 = parseInt(a.getAttribute('data-located')), tmp2 = parseInt(b.getAttribute('data-located'));
    if (tmp1 != tmp2)
        return tmp2 - tmp1;
...
function chat_get_users_frame() {
    var obj = null;
    if (!_top().chat || !_top().chat.chat_user)
        return false;
    if (_top().chat.chat_user.document && _top().chat.chat_user.loaded)
        obj = _top().chat.chat_user.document;
    return obj;
}
function chat_users_can_change(frame) {
    if (frame) {
        ...
        if ($chat_user_list && !$chat_user_list.find('.area.selected').length)
            return false;
        if (session.deaf)
            return false;
    }
...
function chat_users_add(id, kind, raid, user_html_friend, user_html_enemy, force) {
    ...
    if (!chat_users_can_change(frame))
        return;
    if (!force && id == CHAT.my_id && $('#chat_users_list #chat_user_' + id, frame).length)
        return;
    var user_html = '';
    raid = parseInt(raid);
    if (raid && raid == parseInt(session.raid))
        user_html = user_html_friend;
    else if (raid)
        user_html = user_html_enemy;
    else if (kind == session.kind)
        user_html = user_html_friend;
    else
        user_html = user_html_enemy;
...
function chat_users_update(id, kind, raid, user_html_friend, user_html_enemy) {
    ...
    if (!chat_users_can_change(frame))
        return;
    if (!$('#chat_users_list #chat_user_' + id, frame).length)
        return;
    var user_html = '';
    raid = parseInt(raid);
    if (raid && raid == parseInt(session.raid))
...
function controller_party_conf(data) {
    if (_top && _top().frames['chat'])
        _top().frames['chat']['party_leader'] = (data['party|conf'].is_party_leader);
}
...
function replace_user_list(users) {
    ...
    for (var i in users) {
        ...
        var html = '';
        if (raid && raid == parseInt(session.raid))
            html = user['html_friend'];
        else if (raid)
            html = user['html_enemy'];
        else if (kind == session.kind)
            html = user['html_friend'];
        else
            html = user['html_enemy'];
...
function replace_chat_users(users) {
    var frame = chat_get_users_frame(), user_list = $('#chat_users_list .chat_user_item', frame), scrolled_users = [], replaced_users_count = 0, current_scroll = 0, last_visible_user_scroll = 0;
    ...
    if (object.request_statistics) {
        ...
        for (var i in object.request_statistics) {
            if (i == 0) {
                ...
                console_log(object.request_statistics[i]);
...
function controller_user_bag_kinds_diff(object) {
    ...
    if (!object || typeof (object) != 'object' || !object.kind_ids)
        return;
    var kind_ids = [];
    for (var i in object.kind_ids)
        kind_ids.push(object.kind_ids[i]);
...
function controller_bg_update_daystate(obj) {
    ...
    var state_keys = Object.keys(states), state_data, bg_state = 0;
